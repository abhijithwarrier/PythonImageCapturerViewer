...
# IN ADDITION TO CAPTURING AN IMAGE FROM WEBCAM, THIS APPLICATION CAN ALSO BE USED AS AN IMAGE VIEWER TO VIEW IMAGES
# FROM YOUR COMPUTER
#
...
def CreateWidgets():
    root.feedlabel = Label(root, bg="tan4", fg="white", text="WEBCAM FEED", font=('Comic Sans MS',20))
    root.feedlabel.grid(row=1, column=1, padx=10, pady=10, columnspan=2)

    root.cameraLabel = Label(root, bg="tan4", borderwidth=3, relief="groove")
    ...
    root.previewlabel = Label(root, bg="tan4", fg="white", text="IMAGE PREVIEW", font=('Comic Sans MS',20))
...
def ShowFeed():
    ...
    if ret:
        ...
        cv2.putText(frame, datetime.now().strftime('%d/%m/%Y %H:%M:%S'), (20,30), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255,0,0))
...
def Capture():
    ...
    cv2.putText(frame, datetime.now().strftime('%d/%m/%Y %H:%M:%S'), (430,460), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255,0,0))
...
def StopCAM():
    ...
    root.cameraLabel.config(text="CAMERA IS OFF", font=('Comic Sans MS',79))
